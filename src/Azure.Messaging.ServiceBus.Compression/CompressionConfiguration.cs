...
namespace Azure.Messaging.ServiceBus.Compression
{
    ...
    {
        readonly Dictionary<string, Func<byte[], byte[]>> _mutableDecompressors;
        ...
        public CompressionConfiguration(string compressionMethodName, Func<byte[], byte[]> compressor, int minimumSize, Dictionary<string, Func<byte[], byte[]>> decompressors)
        {
            ...
            _mutableDecompressors = decompressors;
        ...
        Func<string, byte[], byte[]> GetSafeDecompressor(Dictionary<string, Func<byte[], byte[]>> userProvidedDecompressors)
        {
            return (compressionMethodName, bytes) =>
            {
                ...
                {
                    throw new Exception($"{nameof(Azure.Messaging.ServiceBus.Compression)} has not been configured to handle messages compressed using '{compressionMethodName}', but a message with this compression has been received. Re-configure plugin to handle '{compressionMethodName}' for decompression.");
                }
        ...
        {
            _mutableDecompressors.Add(compressionMethodName, decompressor);
        }
    }
}